...
(function() {
    ...
    function initFabMic() {
        const fab = document.querySelector('.fab-mic');
        if (!fab) return;

        const existingVoiceBtn = document.querySelector(
            '.voice-btn, .record-btn, [class*="voice-btn"], [id*="voice"]'
        );

        fab.addEventListener('click', () => {
            if (existingVoiceBtn) {
                existingVoiceBtn.click();
            }
            fab.classList.toggle('recording');
        });
    }
